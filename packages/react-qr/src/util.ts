...
export function createSignPayload (address: string, cmd: number, payload: string | Uint8Array, genesisHash: string | Uint8Array, specVersion: number): Uint8Array {
  const specVersionString = specVersion.toString().padStart(6, '0');
  return u8aConcat(
    SUBSTRATE_ID,
    CRYPTO_SR25519,
    new Uint8Array([cmd]),
